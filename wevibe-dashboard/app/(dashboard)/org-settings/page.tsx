    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-8">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-wv-dim">
          {!activeOrg
            ? 'Select or create an org to manage its settings.'
            : isLeader
              ? `Leader controls for ${activeOrg.org_name}.`
              : `Read-only organization status for ${activeOrg.org_name}.`}
        </p>
      </header>
      ...
      {!activeOrg ? (
        ...
        <>
          ...
          {!isLeader && activeOrg && (
            <section className="rounded-xl border border-wv-line bg-wv-panel p-6 shadow-wv-sm">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-semibold text-wv-text">Organization</h2>
              </div>
              <p className="mt-1 text-sm text-wv-dim">
                Organization configuration is managed by the org leader.
              </p>

              <div className="mt-4 rounded-lg border border-wv-line bg-wv-panel-2 p-4">
                <h3 className="text-sm font-semibold uppercase tracking-wide text-wv-dim">Org</h3>
                <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Name</dt>
                    <dd className="mt-1 text-sm text-wv-text">{activeOrg.org_name}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Org ID</dt>
                    <dd className="mt-1 font-mono text-sm text-wv-text break-all">{activeOrg.org_id}</dd>
                  </div>
                </dl>
              </div>

              <div className="mt-4 rounded-lg border border-wv-line bg-wv-panel-2 p-4">
                <h3 className="text-sm font-semibold uppercase tracking-wide text-wv-dim">Your Membership</h3>
                <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Role</dt>
                    <dd className="mt-1 text-sm text-wv-text">{activeOrg.role}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Can contribute</dt>
                    <dd className="mt-1 text-sm text-wv-text">
                      {activeOrg.can_contribute === undefined ? '—' : activeOrg.can_contribute ? 'Yes' : 'No'}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Can moderate</dt>
                    <dd className="mt-1 text-sm text-wv-text">
                      {activeOrg.can_moderate === undefined ? '—' : activeOrg.can_moderate ? 'Yes' : 'No'}
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="mt-4 rounded-lg border border-wv-line bg-wv-panel-2 p-4">
                ...
                <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Allowed providers</dt>
                    <dd className="mt-1 text-sm text-wv-text break-all">
                      {activeOrg.allowed_providers.length > 0 ? activeOrg.allowed_providers.join(', ') : '—'}
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              {orgSummary && (
                <div className="mt-4 rounded-lg border border-wv-line bg-wv-panel-2 p-4">
                  <h3 className="text-sm font-semibold uppercase tracking-wide text-wv-dim">Organization Summary</h3>
                  <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                      <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Domain</dt>
                      <dd className="mt-1 text-sm text-wv-text">{orgSummary.domain || '—'}</dd>
                    </div>
                    <div>
                      <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Status</dt>
                      <dd className="mt-1 text-sm text-wv-text">{orgSummary.status || '—'}</dd>
                    </div>
                    <div>
                      <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Description</dt>
                      <dd className="mt-1 text-sm text-wv-text">{orgSummary.description || '—'}</dd>
                    </div>
                    <div>
                      <dt className="text-xs font-semibold uppercase tracking-wide text-wv-dim">Created at</dt>
                      <dd className="mt-1 text-sm text-wv-text">{orgSummary.created_at || '—'}</dd>
                    </div>
                  </dl>
                </div>
              )}
