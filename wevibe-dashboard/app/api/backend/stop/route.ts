...
import { getBackendStatus, stopBackend } from '@/lib/backend-control';
...
export async function POST(request: Request): Promise<Response> {
  ...
  logOp('dashboard.backend_stop', 'info', {
    trace,
    phase: 'entry',
  });
  ...
  try {
    ...
    logOp('dashboard.backend_stop', result.ok ? 'info' : 'error', {
      trace,
      phase: 'outcome',
    ...
    logOp('dashboard.backend_stop', 'error', {
      trace,
      phase: 'outcome',
      status: 'error',
