...
import { getBackendStatus, restartBackend } from '@/lib/backend-control';
...
export async function POST(request: Request): Promise<Response> {
  ...
  logOp('dashboard.backend_restart', 'info', {
    trace,
    phase: 'entry',
  });

  try {
    const result = await restartBackend(trace);

    logOp('dashboard.backend_restart', result.ok ? 'info' : 'error', {
      trace,
      phase: 'outcome',
    ...
    logOp('dashboard.backend_restart', 'error', {
      trace,
      phase: 'outcome',
      status: 'error',
