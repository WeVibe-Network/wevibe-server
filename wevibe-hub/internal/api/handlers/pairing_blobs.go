...
func StorePairingBlob(w http.ResponseWriter, r *http.Request) {
	if pool == nil {
	...
	if req.PairingID == "" || req.HkdfSalt == "" || req.IV == "" || req.Ciphertext == "" {
		http.Error(w, `{"error":"missing required fields"}`, http.StatusBadRequest)
		return
	}

	if err := envelopes.StorePairingBlob(r.Context(), pool, req.PairingID, req.HkdfSalt, req.IV, req.Ciphertext); err != nil {
...
func GetPairingBlob(w http.ResponseWriter, r *http.Request) {
	if pool == nil {
		http.Error(w, `{"error":"database unavailable"}`, http.StatusServiceUnavailable)
		return
	}
	...
	if pairingID == "" {
		http.Error(w, `{"error":"missing pairing_id"}`, http.StatusBadRequest)
		return
	}

	hkdfSalt, iv, ciphertext, err := envelopes.ConsumePairingBlob(r.Context(), pool, pairingID)
	if err != nil {
